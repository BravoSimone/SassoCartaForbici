class ManageMatchesController < ApplicationController
  def generate_match
    ...
    all_matches = current_user.all_matches(current_user).to_a
    ...
    if @match == nil
      new_match = { :player_1_id => current_user.id, :player_2_id => current_user.get_user_by_email(@opponent).to_a[0][:id],
                    :p1_win => 0, :p2_win => 0}
